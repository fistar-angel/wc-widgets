...
$(document).ready(function(){
    // log message
    MashupPlatform.widget.log("Indivindual Fall risk filter iwidget loaded", MashupPlatform.log.INFO);
	// local variables
...
$(document).on("click","#submit_button", function(){
	...
	var risk = ($("#risk_thresh").val() == "") ? 0 : $("#risk_thresh").val();
	var object = {user: $("#select_user").val(), parameter: $("#select_param").val(), risk: risk, start: $("#from_date").val(), end: $("#until_date").val()}
	...
	// Handle invalid data
	if (state === 0){
		//alert("No valid user input. Please check the selected dates or select a user/biologic parameter!");
		...
		MashupPlatform.wiring.pushEvent("paramID", object["parameter"]);
		MashupPlatform.wiring.pushEvent("startDate", object["start"]);
		MashupPlatform.wiring.pushEvent("endDate", object["end"]);
		MashupPlatform.wiring.pushEvent("riskThreshold", object["risk"]);
...
$(document).on("click","#download-risk-btn", function(){
	// declare local JS object
	var risk = ($("#risk_thresh").val() == "") ? 0 : $("#risk_thresh").val();
	var object = {user: $("#select_user").val(), parameter: $("#select_param").val(), risk: risk, start: $("#from_date").val(), end: $("#until_date").val()}
	// validate user input

	if (validation(object) === 0){
	...
	// url
	global['url'] = global['baseURL'] + "/users/" + object['user'] + "/biologic_parameter/" + object['parameter'] + "/risk";
	global['url'] += '/' + object["start"] + '/' + object["end"] + '/' + global["username"] + "&threshold="+ object["risk"];


	// set file name
	var id = $("#select_user").find('option:selected').text().split(" ");
	var filename = "RISK_" + id[0] + '_' + $("#select_param").find('option:selected').text();
	...
	MashupPlatform.http.makeRequest( global['url'], {
		...
		onSuccess: function(input) {
			if (input["status"] === 200) {
				...
				json2csv(response.response.risk, filename);
			}
		},
		onFailure: function() {
...
function json2csv(risk, filename){
    var CSV = '';
    var header = "Falling risk,Date/time";	

    //append Label row with line break
    CSV += header + '\r\n';

    for (var i in risk) {
    	CSV += risk[i].value.toString() + ' %,';
    	CSV += risk[i].timestamp.toString();
    	CSV += '\r\n';
    }
...
//**************************
//	Check the risk value
//**************************
function validateRisk(risk){
	if (risk < 0 || risk > 100 || isNaN(risk)){
		$("#risk_thresh").parent().addClass("has-error");
		return 0;
	}
	$("#risk_thresh").parent().removeClass("has-error");
	return 1;
}
